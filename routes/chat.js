...
router.post("/rooms", async (req, res) => {
  ...
  try {
    ...
    const participantIdList = participants.map((participant) => participant.member_id);
    ...
    const roomName =
      name ||
      participants
        .map((participant) => participant.nickname || participant.name || `회원${participant.member_id}`)
        .join(", ");
