...
export const backendConfig = {
  ...
  endpoints: {
    ...
    policies: process.env.EXPO_PUBLIC_POLICIES_PATH ?? '/api/policies',
    policiesRecommended:
    ...
    policiesHistory: process.env.EXPO_PUBLIC_POLICIES_HISTORY_PATH ?? '/api/policies/history',
...
export function mapBackendPost(raw: any, fallbackLocation?: NeighborhoodLocation): Post {
  ...
  return {
    ...
    createdFrom: raw?.createdFrom ?? raw?.created_from,
...
export function mergeCreatedPost(
  ...
  return {
    ...
    createdFrom: mapped.createdFrom ?? (payload.type === 'need' ? 'app' : undefined),
